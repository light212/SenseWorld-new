...
- **OpenAISpeechProvider（新建）**: 实现 `SpeechProvider`，封装 OpenAI SDK 调用
...
- Azure Speech 使用 REST API（Node.js 20 内置 `fetch`），不使用 `microsoft-cognitiveservices-speech-sdk`（该 SDK 与 Next.js 14 App Router Webpack 打包不兼容，含原生 node-gyp 模块）
